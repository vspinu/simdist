...
NumericMatrix C_dense_dist(std::string dist_type,
                           ...
                           bool self = false) {
  if (dist_type == "COSINE")
    return denseDist<COSINE>(XIX, xmat, YIX, ymat, self);
  else if (dist_type == "EUCLIDEAN")
    return denseDist<EUCLIDEAN>(XIX, xmat, YIX, ymat, self);
  else
...
NumericMatrix C_sparse_dist(std::string dist_type,
                            ...
                            const bool self = false, const int xreorder = 0, const int yreorder = 0) {
  if (dist_type == "COSINE") 
    return sparseDist<COSINE>(XIX, XP, XV, YIX, YP, YV, out_rows, out_cols, self, xreorder, yreorder);
  else if (dist_type == "EUCLIDEAN")
    return sparseDist<EUCLIDEAN>(XIX, XP, XV, YIX, YP, YV, out_rows, out_cols, self, xreorder, yreorder);
  else
    throw std::invalid_argument("Invalid dist_type: " + dist_type);
}
...
NumericMatrix C_triplet_dist(std::string dist_type,
                             ...
                             const bool self = false, const int xreorder = 0, const int yreorder = 0) {
  if (dist_type == "COSINE") 
    return tripletDist<COSINE>(xpri, xsec, xval, ypri, ysec, yval, out_rows, out_cols, self, xreorder, yreorder);
  else if (dist_type == "EUCLIDEAN")
    return tripletDist<EUCLIDEAN>(xpri, xsec, xval, ypri, ysec, yval, out_rows, out_cols, self, xreorder, yreorder);
  else
    throw std::invalid_argument("Invalid dist_type: " + dist_type);
}

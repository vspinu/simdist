...
##' @export
psv2mat <- function(df) {
    pid <- as.factor(primary_ids(df))
    sid <- as.factor(secondary_ids(df))
    val <- sparse_vals(df)
    sparseMatrix(i = as.integer(pid), j = as.integer(sid),
                 dims = c(length(levels(pid)), length(levels(sid))),
                 dimnames = list(levels(pid), levels(sid)))
}
